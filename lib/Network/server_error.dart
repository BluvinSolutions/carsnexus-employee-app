...
class ServerError implements Exception {
  ...
  _handleError(DioException error) {
    if(error.response!.statusCode == 401) {
      ...
      {
        return DeviceUtils.toastMessage('${error.response!.data['errors']['old_password'][0]}');
      }
    else if(error.response!.data['errors']['password']!=null)
      {
        return DeviceUtils.toastMessage('${error.response!.data['errors']['password'][0]}');
      }
    else if(error.response!.data['errors']['phone_no'] != null)
    {
      return DeviceUtils.toastMessage('${error.response!.data['errors']['phone_no'][0]}');
    }
    else if(error.response!.data['errors']['email'] != null)
    {
      return DeviceUtils.toastMessage('${error.response!.data['errors']['email'][0]}');
    }
